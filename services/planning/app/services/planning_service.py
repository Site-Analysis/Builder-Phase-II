...
NBC_FAR: dict[str, dict[tuple[float, float], float]] = {
    "Residential":   {(0, 7.5): 1.00, (7.5, 12): 1.50, (12, 18): 2.00, (18, 24): 2.50, (24, float("inf")): 3.00},
    "Commercial":    {(0, 7.5): 1.50, (7.5, 12): 2.00, (12, 18): 2.50, (18, 24): 3.00, (24, float("inf")): 3.50},
...
NBC_GROUND_COVERAGE: dict[str, float] = {
    "Residential": 0.55, "Commercial": 0.60, "Industrial": 0.65,
    "Mixed Use": 0.55, "Institutional": 0.45,
}

NBC_MAX_HEIGHT: dict[str, float] = {
    "Residential": 45.0, "Commercial": 70.0, "Industrial": 45.0,
    "Mixed Use": 70.0, "Institutional": 45.0,
}
...
async def _detect_road_width(lat: float, lon: float, client: httpx.AsyncClient) -> tuple[float, str]:
    """
...
"""
    try:
        ...
        for el in elements:
            ...
            if (tags.get("railway") == "station" or tags.get("station") == "subway") and d < station_dist:
                station_dist = d
...
class PlanningService:
    async def analyze(self, request: PlanningRequest) -> PlanningResult:
        ...
        async with httpx.AsyncClient(timeout=15, headers={"User-Agent": "SAT-SiteAnalysisTool/1.0"}) as client:
